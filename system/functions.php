<?php
function checkPermission($edit, $addP, $editP){
	if($edit) return has_permission($editP);
	else return has_permission($addP);
}
...
function datetime_to_db($date, $explode_by = '-', $join_by = '-')
{
    if (!$date) return null;
...
function datetime_to_user($date, $explode_by = '-', $join_by = '-')
{
    if (!$date) return null;
    list($date, $time) = explode(' ', $data);
...
{
    return array('error' => array(), 'success' => array());
}
...
function generateIntelImgTag($img, $sizeParam, $class = '', $id = '', $alt = '', $title = '', $path_folder = 'uploads', $save_dir = null, $alternatives = array())
{
    ...
    <img src="<?php echo get_image($img, $sizeParam) ?>" class="<?php echo $class; ?>" id="<?php echo $id; ?>" title="<?php echo $title; ?>" alt="<?php echo $alt; ?>" />
<?php
}
/**********/
...
{
    return implode($glue, array_filter($data, function ($var) {
        return !($var == "" || $var == null);
    }));
}
/*************/
