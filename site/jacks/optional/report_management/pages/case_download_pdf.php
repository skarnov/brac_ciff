...
if (!$report):
    add_notification('Please Select A Portion', 'warning');
    header('location: ' . url('admin/dev_report_management/case_report'));
    exit();
endif;
...
$case_info = $customers->get_cases($args);
...
$family_counsellings = $customers->get_family_counselling(array('customer_id' => $customer_id));
$psychosocial_sessions = $customers->get_psychosocial_session(array('customer_id' => $customer_id));
$psychosocial_completions = $customers->get_psychosocial_completion(array('customer_id' => $customer_id));
$psychosocial_followups = $customers->get_psychosocial_followup(array('customer_id' => $customer_id));
$reviews = $customers->get_case_review(array('customer_id' => $customer_id));
...
if (in_array('immediate_support', $report)) :
...
if (in_array('reintegration_plan', $report)) :
    $devPdf->SetFont('Times', 'B', 14);
...
if (in_array('psychosocial_support', $report)) :
    $devPdf->SetFont('Times', 'B', 14);
...
if (in_array('family_counseling', $report)) :
    $devPdf->SetFont('Times', 'B', 14);
...
if (in_array('reintegration_session', $report)) :
    $devPdf->SetFont('Times', 'B', 14);
...
if (in_array('session_completion', $report)) :
    $devPdf->SetFont('Times', 'B', 14);
...
if (in_array('economic_reintegration', $report)) :
    $devPdf->SetFont('Times', 'B', 14);
...
if (in_array('economic_referrals', $report)) :
    $devPdf->SetFont('Times', 'B', 14);
...
if (in_array('social_reintegration', $report)) :
    $devPdf->SetFont('Times', 'B', 14);
