...
if ($filter_branch) {
    $sql = "SELECT full_name, customer_id, present_house, present_road, present_village, present_post_office, present_police_station, present_sub_district, present_district FROM dev_customers WHERE fk_branch_id = '$filter_branch'";
...
if ($_GET['download_csv']) {
    ...
    $sql = "SELECT full_name, customer_id, present_house, present_road, present_village, present_post_office, present_police_station, present_sub_district, present_district FROM dev_customers WHERE fk_branch_id = '$filter'";
    $results = $devdb->get_results($sql);
    ...
    $csvFile = $csvFolder . 'branch_wise_beneficiary-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('', 'Branch Wise Beneficiary Report', '');
...
<div class="page-header">
    <h1>Branch Wise Beneficiary</h1>
    <div class="oh">
        <div class="btn-group btn-group-sm">
            <?php
            echo linkButtonGenerator(array(
                'href' => '?download_csv=1&filter=' . $filter_branch,
                'attributes' => array('target' => '_blank'),
                'action' => 'download',
                'icon' => 'icon_edit',
                'text' => 'Download Branch Wise Beneficiary',
                'title' => 'Download Branch Wise Beneficiary Report',
            ));
            ?>
        </div>
    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="branch">
                            ...
                            foreach ($all_branches['data'] as $value) :
                                ...
                                if ($value['pk_branch_id'] == $filter_branch) {
                                    echo 'selected';
                                }
