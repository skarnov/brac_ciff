...
$pre_data = array();
if ($edit) {
    $pre_data = $this->get_courses(array('course_id' => $edit, 'select_fields' => array(
            'dev_courses.fk_branch_id',
            'dev_courses.course_name',
            'dev_courses.course_duration',
            'dev_courses.course_price',
        ), 'single' => true));

    if (!$pre_data) {
        add_notification('Invalid course, no data found.', 'error');
...
$pagination = pagination($courses['total'], $per_page_items, $start);
...
if ($_POST) {
    ...
    $ret = $this->add_edit_course($data);

    if ($ret['success']) {
        $msg = "Information of course has been " . ($edit ? 'updated.' : 'saved.');
        add_notification($msg);
        $activityType = $edit ? 'update' : 'create';
        user_activity::add_activity($msg, 'success', $activityType);
        header('location: ' . url('admin/dev_course_management/manage_courses'));
        exit();
    } else {
        $pre_data = $_POST;
...
if ($_GET['download_csv']) {
    ...
    $report_title = array('', 'Course Report', '');
    ...
    $headers = array('#', 'Branch Name', 'Course Name', 'Duration (Hours) ', 'Price (BDT)');
    fputcsv($fh, $headers);
...
<div class="row">
            ...
            <div class="panel">
                <div class="panel-body">                    
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="course_name" value="<?php echo $pre_data['course_name'] ? $pre_data['course_name'] : ''; ?>" required="">
                    </div>
                    <div class="form-group">
                        <label>Duration (Hours)</label>
                        <input class="form-control" type="text" name="course_duration" value="<?php echo $pre_data['course_duration'] ? $pre_data['course_duration'] : ''; ?>">
                    </div>
                    <div class="form-group">
                        <label>Price (BDT)</label>
                        <input class="form-control" type="number" step="0.01" name="course_price" value="<?php echo $pre_data['course_price'] ? $pre_data['course_price'] : ''; ?>">
                    </div>
                </div>
                <div class="panel-footer tar">
                    <a href="<?php echo url('admin/dev_course_management/manage_courses') ?>" class="btn btn-flat btn-labeled btn-danger"><span class="btn-label icon fa fa-times"></span>Cancel</a>
                    <?php
                    echo submitButtonGenerator(array(
                        'action' => $edit ? 'update' : 'update',
                        'size' => '',
                        'id' => 'submit',
                        'title' => $edit ? 'Update The Course' : 'Save The Course',
    ...
    <div class="col-md-8">
        <div class="table-primary table-responsive">
            ...
            <?php endif; ?>
            <div class="table-header">
                <?php echo searchResultText($courses['total'], $start, $per_page_items, count($courses['data']), 'courses') ?>
            </div>
            <table class="table table-bordered table-condensed">
                ...
                <tbody>
                    <?php
                    foreach ($courses['data'] as $i => $course) {
                        ?>
                        <tr>
                            <td><?php echo $course['branch_name'] ?></td>
                            ...
                            <td class="tar action_column">
                                <?php if (has_permission('edit_course')): ?>
                                    <div class="btn-toolbar">
                                        <?php
                                        echo linkButtonGenerator(array(
                                            'href' => build_url(array('action' => 'manage_courses', 'edit' => $course['pk_course_id'])),
                                            'action' => 'edit',
                                            'icon' => 'icon_edit',
                                            'text' => 'Edit',
                                            'title' => 'Edit Course',
