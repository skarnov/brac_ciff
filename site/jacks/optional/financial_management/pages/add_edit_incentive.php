...
$pre_data = array();
if ($edit) {
    $pre_data = $this->get_sales_incentive(array('branch_id' => $branch_id, 'incentive_id' => $edit, 'select_fields' =>array(
        'dev_sales_incentive.fk_project_id'
        ,'dev_sales_incentive.fk_branch_id'
        ,'dev_sales_incentive.fk_staff_id'
        ,'dev_sales_incentive.month_name'
        ,'dev_sales_incentive.unit_incentive'
        ,'dev_sales_incentive.incentive_percentage'
    ), 'single' => true));
    if (!$pre_data) {
        add_notification('Invalid incentive, no data found.', 'error');
...
if ($_POST) {
    ...
    $data['edit'] = $edit;
    $ret = $this->add_edit_incentive($data);

    if ($ret['success']) {
        $msg = "Incentive " . ($edit ? 'updated.' : 'saved.');
        add_notification($msg);
        $activityType = $edit ? 'update' : 'create';
        user_activity::add_activity($msg, 'success', $activityType);
        header('location: ' . url('admin/dev_financial_management/manage_incentive_management'));
        exit();
    } else {
        $pre_data = $_POST;
    ...
    <div class="oh">
        <div class="btn-group btn-group-sm">
            ...
            echo linkButtonGenerator(array(
                'href' => 'manage_incentive_management',
                'action' => 'list',
                'icon' => 'icon_list',
                'text' => 'Manage Incentives',
                'title' => 'Manage Sales Incentive',
            ));
            ?>
        </div>
    </div>
        ...
        <div class="panel">
            <div class="panel-body">
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        <label>Unit Incentive</label>
                        <input required class="form-control" type="number" name="unit_incentive" value="<?php echo $pre_data['unit_incentive'] ? $pre_data['unit_incentive'] : ''; ?>">
                    </div>
                    <div class="form-group">
                        <label>Incentive Percentage</label>
                        <input required class="form-control" type="number" name="incentive_percentage" value="<?php echo $pre_data['incentive_percentage'] ? $pre_data['incentive_percentage'] : ''; ?>">
                    </div>
                </div>
            </div>
            <div class="panel-footer tar">
                ...
                echo submitButtonGenerator(array(
                    ...
                    'size' => '',
                    'id' => 'submit',
                    'title' => $edit ? 'Update Incentive' : 'Save Incentive',
                    'icon' => $edit ? 'icon_update' : 'icon_save',
                    'text' => $edit ? 'Update Incentive' : 'Save Incentive'))
                ?>
            </div>
        </div>
    </form>
