...
$args = array(
    'select_fields' => array(
        'dev_sales_targets.pk_target_id',
...
if ($_GET['download_csv']) {
    ...
    $csvFile = $csvFolder . 'salesTarget-' . time() . '.csv';

    $fh = fopen($csvFile, 'w');

    $report_title = array('', 'Sales Target Report', '');
    fputcsv($fh, $report_title);

    $blank_row = array('');
    fputcsv($fh, $blank_row);

    $headers = array('#', 'Date', 'Branch', 'Staff Name', 'Month', 'Target Unit');
    fputcsv($fh, $headers);
...
<div class="page-header">
    <h1>All Sales Target</h1>
    <div class="oh">
        <div class="btn-group btn-group-sm">
            <?php
            echo linkButtonGenerator(array(
                'href' => build_url(array('action' => 'add_edit_target')),
                'action' => 'add',
                'text' => 'Add Target',
                'title' => 'Manage All Targets',
                'icon' => 'icon_add',
                'size' => 'sm'
            ));
            ?>
            ...
            <table class="table table-bordered table-condensed">
                <thead>
                    <tr>
                        ...
                        <th style="width: 10%; text-align: right">Target Quantity</th>
                            ...
                            <td class="tar action_column">
                                <?php if (has_permission('edit_financial')): ?>
                                    <div class="btn-toolbar">
                                        <?php
                                        echo linkButtonGenerator(array(
                                            'href' => build_url(array('action' => 'add_edit_target', 'edit' => $v_target['pk_target_id'])),
                                            'action' => 'edit',
                                            'icon' => 'icon_edit',
                                            'text' => 'Edit',
                                            'title' => 'Edit Target',
