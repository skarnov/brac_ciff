...
$training_validation = $this->get_sharing_sessions(array('training_id' => $training_id, 'single' => true));
...
$devPdf->Cell(0, 10, 'Date: ' . $trainings['date'] ? date('d-m-Y', strtotime($trainings['date'])) : 'N/A', 0, 1, 'L');
...
if ($training_validation['data'] != NULL) {
    foreach ($training_validation['data'] as $i => $item) {
        ...
        $devPdf->SetFont('Times', '', 12);
        $devPdf->Cell(0, 10, 'Training Date: ' . $item['traning_date'] ? date('d-m-Y', strtotime($item['traning_date'])) : 'N/A', 0, 1, 'L');
        $devPdf->Cell(0, 10, 'Evaluator Profession: ' . ucfirst($item['evaluator_profession']), 0, 1, 'L');
        ...
        $devPdf->Cell(0, 10, 'How satisfied are you with the training venue and other logistic supports: ' . $item['satisfied_supports'], 0, 1, 'L');
        ...
        $devPdf->Cell(0, 10, 'What extent your knowledge increased on trafficking law: ' . $item['trafficking_law'], 0, 1, 'L');
        $devPdf->Cell(0, 10, 'What extent your knowledge increased on policy process: ' . $item['policy_process'], 0, 1, 'L');
        $devPdf->Cell(0, 10, 'What extent your knowledge increased on over all contents: ' . $item['all_contents'], 0, 1, 'L');
