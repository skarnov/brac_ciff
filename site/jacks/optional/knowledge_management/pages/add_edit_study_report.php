...
if (!checkPermission($edit, 'add_study_report', 'edit_study_report')) {
...
if ($edit) {
    ...
    if (!$pre_data) {
        add_notification('Invalid study report, no data found.', 'error');
...
if ($_POST) {
    ...
    $ret = $this->add_edit_study_report($data);

    if ($ret) {
        $msg = "Study Report has been " . ($edit ? 'updated.' : 'saved.');
        add_notification($msg);
        $activityType = $edit ? 'update' : 'create';
        user_activity::add_activity($msg, 'success', $activityType);
        if ($edit) {
            header('location: ' . url('admin/dev_knowledge_management/manage_study_reports?action=add_edit_study_report&edit=' . $edit));
        } else {
            header('location: ' . url('admin/dev_knowledge_management/manage_study_reports'));
        }
        exit();
    } else {
        $pre_data = $_POST;
...
$all_study_report_tags = $this->get_lookups('success_study_report');
...
<form id="theForm" onsubmit="return true;" method="post" action="" enctype="multipart/form-data">
    <div class="panel" id="fullForm" style="">
                ...
                <div class="form-group">
                    ...
                    <div class="form_element_holder radio_holder radio_holder_static_featured_show_link">
                        <div class="options_holder radio">
                            ...
                            foreach ($all_study_report_tags['data'] as $tag) {
                                ?>
        ...
        <div class="panel-footer tar">
            <a href="<?php echo url('admin/dev_knowledge_management/manage_study_reports') ?>" class="btn btn-flat btn-labeled btn-danger"><span class="btn-label icon fa fa-times"></span>Cancel</a>
            <?php
