...
if (!checkPermission($edit, 'add_organogram', 'edit_organogram')) {
...
if ($edit) {
    ...
    if (!$pre_data) {
        add_notification('Invalid organogram, no data found.', 'error');
...
if ($_POST) {
    ...
    $ret = $this->add_edit_organogram($data);

    if ($ret) {
        $msg = "Organogram has been " . ($edit ? 'updated.' : 'saved.');
        add_notification($msg);
        $activityType = $edit ? 'update' : 'create';
        user_activity::add_activity($msg, 'success', $activityType);
        if ($edit) {
            header('location: ' . url('admin/dev_knowledge_management/manage_organograms?action=add_edit_organogram&edit=' . $edit));
        } else {
            header('location: ' . url('admin/dev_knowledge_management/manage_organograms'));
        }
        exit();
    } else {
        $pre_data = $_POST;
...
$all_organogram_tags = $this->get_lookups('success_organogram');
...
<div class="page-header">
    <h1><?php echo $edit ? 'Update ' : 'New ' ?> Organogram </h1>
    <div class="oh">
        <div class="btn-group btn-group-sm">
            ...
            echo linkButtonGenerator(array(
                'href' => $myUrl,
                'action' => 'list',
                'text' => 'All Organograms',
                'title' => 'Manage Organograms',
                'icon' => 'icon_list',
                'size' => 'sm'
            ));
            ?>
...
<form id="theForm" onsubmit="return true;" method="post" action="" enctype="multipart/form-data">
    <div class="panel" id="fullForm" style="">
                ...
                <div class="form-group">
                    ...
                    <div class="form_element_holder radio_holder radio_holder_static_featured_show_link">
                        <div class="options_holder radio">
                            ...
                            foreach ($all_organogram_tags['data'] as $tag) {
                                ?>
        ...
        <div class="panel-footer tar">
            <a href="<?php echo url('admin/dev_knowledge_management/manage_organograms') ?>" class="btn btn-flat btn-labeled btn-danger"><span class="btn-label icon fa fa-times"></span>Cancel</a>
            <?php
