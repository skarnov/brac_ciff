...
<div class="table-primary table-responsive">
    ...
    <table class="table table-bordered table-condensed">
        ...
        <tbody>
            <?php
            foreach ($assessment_reports['data'] as $i => $report) {
                ?>
                <tr>
                    ...
                    <td class="tar action_column">
                        <?php if (has_permission('edit_assessment_report')): ?>
                            <div class="btn-group btn-group-sm">
                                <?php
                                echo linkButtonGenerator(array(
                                    'href' => build_url(array('action' => 'add_edit_assessment_report', 'edit' => $report['pk_knowledge_id'])),
                                    'action' => 'edit',
                                    'icon' => 'icon_edit',
                                    'text' => 'Edit',
                                    'title' => 'Edit Assessment Report',
                                ));
                                ?>
                            </div>
                        <?php endif; ?>
                        <?php if (has_permission('delete_assessment_report')): ?>
                            <div class="btn-group btn-group-sm">
                                <?php
        ...
        $(document).on('click', '.delete_single_record', function () {
            ...
            bootbox.prompt({
                ...
                callback: function (result) {
                    if (result == 'delete') {
                        window.location.href = '?action=deleteAssessmentReport&id=' + logId;
                    }
